...
if __name__ == '__main__': 
    ...
    train_folders = sorted([f for f in os.listdir(detect_path) if os.path.isdir(detect_path / f) and f.startswith("train")])
    if len(train_folders) == 0:
        raise ValueError("No training folders found")
    # Automatically select the last training folder
